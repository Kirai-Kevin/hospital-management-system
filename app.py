...
def index():
    ...
    upcoming_appointments = Appointment.query.filter(Appointment.time > datetime.now()).count()
...
def delete_patient(patient_id):
    try:
        ...
        if patient:
            ...
            BillingRecord.query.filter_by(patient_id=patient_id).delete()
        ...
        db.session.rollback()
        flash(f'An error occurred while deleting the patient: {str(e)}', 'error')
...
def search_patient():
    ...
    return render_template('search_patient.html', patients=patients, searched=searched)
...
def schedule_appointment():
    if request.method == 'POST':
        ...
        patient = Patient.query.get(patient_id)
        if not patient:
            flash('Patient not found. Please add the patient first.', 'error')
